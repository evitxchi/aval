...
If the live smoke fails, the Worker has already been deployed; the desktop release remains blocked until the full workflow succeeds. Anthropic failures now retain a fixed, safe explanation and provider request ID in the task error. `insufficient_credits` and `spend_limit` require action on the API account used by that workspace (the reserved verification account uses the deployment's `ANTHROPIC_API_KEY`). They are not proof of a malformed request and must not be worked around by skipping verification. `invalid_request` means the provider rejected request parameters; use its request ID to investigate. Provider response bodies, which can quote user input, are not copied into logs or task records.
