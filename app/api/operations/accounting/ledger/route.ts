...
export async function GET(request: Request) {
  ...
  const entries = await listLedgerEntries(identity.organizationId, {
    leaseId,
  ...
  return Response.json({
    ...
    balanceCents: leaseId ? await leaseBalanceCents(identity.organizationId, leaseId) : null,
  });
}
...
export async function POST(request: Request) {
  ...
  try {
    ...
    const entry = await postLedgerEntry(identity.organizationId, {
