...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (connection.authMode === "oauth2") {
      if (connection.expiresAt && connection.expiresAt <= new Date()) return Response.json({ error: "The access token expired. Allow the import worker to refresh it or reconnect before validation.", status: "not_tested" }, { status: 409 });
