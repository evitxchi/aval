...
const operationCopy: Record<string, { titleKey: string; subtitleKey: string; metrics: { labelKey: string; value: number; prefix?: string; suffix?: string; decimals?: number }[] }> = {
  properties: { titleKey: "Nav.properties", subtitleKey: "OperationsView.propertiesSubtitle", metrics: [{ labelKey: "Nav.properties", value: 6 }, { labelKey: "OperationsView.unitsCount", value: 142 }, { labelKey: "OperationsView.occupiedCount", value: 134 }, { labelKey: "OperationsView.readyForLeasingCount", value: 5 }] },
  ...
  maintenance: { titleKey: "Nav.maintenance", subtitleKey: "OperationsView.maintenanceSubtitle", metrics: [{ labelKey: "OperationsView.reportedCount", value: 31 }, { labelKey: "OperationsView.assignedCount", value: 24 }, { labelKey: "OperationsView.workDoneCount", value: 18 }, { labelKey: "OperationsView.completedCount", value: 16 }] },
  accounting: { titleKey: "Nav.accounting", subtitleKey: "OperationsView.accountingSubtitle", metrics: [{ labelKey: "OperationsView.rentBilledCount", value: 662, prefix: "$", suffix: "k" }, { labelKey: "OperationsView.collectedCount", value: 613, prefix: "$", suffix: "k" }, { labelKey: "OperationsView.pastDueCount", value: 49, prefix: "$", suffix: "k" }, { labelKey: "OperationsView.collectionRateCount", value: 92.6, suffix: "%", decimals: 1 }] },
...
function OperationsView({ view, openConnections }: { view: View; openConnections: () => void }) { const t = useTranslations(); const data = operationCopy[view]; return <div className="view-wrap"><AppHeader title={t(data.titleKey)} subtitle={t(data.subtitleKey)} actions={<button className="primary-button" onClick={openConnections}><NetworkLeft width={18} height={18}/>{t("OperationsView.connectSource")}</button>}/><section className="metric-grid compact-metrics">{data.metrics.map((metric) => <article className="metric-card" data-reveal key={metric.labelKey}><span>{t(metric.labelKey)}</span><strong><AnimatedNumber value={metric.value} prefix={metric.prefix} suffix={metric.suffix} decimals={metric.decimals}/></strong><div className="empty-spark"/></article>)}</section><section className="panel locked-panel" data-reveal><div className="locked-visual"><div className="locking-lines"><i/><i/><i/></div><span><Key width={23} height={23}/></span></div><div><p className="eyebrow">{t("OperationsView.verifiedDataRequired")}</p><h2>{t("OperationsView.connectTheSystemThatOwnsThis")}</h2><p>{t("OperationsView.avalPreservesSourceIdsCursorsAnd")}</p><button className="primary-button" onClick={openConnections}>{t("OperationsView.openConnections")}<NavArrowRight width={17} height={17}/></button></div></section></div>; }
