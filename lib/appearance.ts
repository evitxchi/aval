...
export type AppearancePreferences = { profile: AvatarSelection | null; agents: Record<string, AvatarSelection>; motion: AvatarMotion };
...
export function parseAppearance(value: unknown): AppearancePreferences | null {
  ...
  return { profile: item.profile && clean(item.profile), agents: Object.fromEntries(entries.map(([id, avatar]) => [id, clean(avatar)])), motion: item.motion };
}
