...
import { agentWorkerRuns } from "@/db/schema";
...
export async function runAgentWorkerBatch(env: AgentWorkerEnv, trigger: WorkerTrigger = "scheduled"): Promise<WorkerBatchResult> {
  const runId = crypto.randomUUID();
  const startedAt = new Date();
  await getDb().insert(agentWorkerRuns).values({
    id: runId,
    trigger,
    status: "running",
    tasksScanned: 0,
  ...
  try {
    const expired = await expireAllStaleApprovals();
    const [ordinary, approval] = await Promise.all([
      claimableTasks(BATCH_SIZE),
      resumableApprovalTasks(BATCH_SIZE),
    ]);
    ...
    const outcomes = await mapWithConcurrency(tasks, CONCURRENCY, (task) =>
      advanceTask(env, task.organizationId, task.id, newWorkerId(), {
        invocationBudgetMs: 35_000,
        maxStepsThisInvocation: 3,
      }),
    );
    const counts = countOutcomes(outcomes);
    const reconciliation = await reconcileDueFinancialOperations(env, 25);
    ...
    await getDb().update(agentWorkerRuns).set({
      status: "failed",
    ...
    await emitAgentOperationalAlert(env, { severity: "critical", code: "worker_failed", runId, occurredAt: new Date().toISOString() })
...
export async function runTaskInBackground(env: AgentWorkerEnv, organizationId: string, taskId: string, trigger: "request" | "approval"): Promise<AdvanceOutcome> {
  const started = Date.now();
  const outcome = await advanceTask(env, organizationId, taskId, newWorkerId(), {
    invocationBudgetMs: 35_000,
    maxStepsThisInvocation: 3,
  });
  ...
  if (outcome.status === "FAILED") {
    await emitAgentOperationalAlert(env, { severity: "warning", code: "task_failed", taskId, occurredAt: new Date().toISOString() })
...
async function finishRun(runId: string, result: WorkerBatchResult): Promise<void> {
  await getDb().update(agentWorkerRuns).set({
    status: "completed",
