...
export async function executionAuthority(org: string, user: string, taskId: string | undefined, tool: string, args: Record<string,unknown>) {
  const state = await readOnboarding(user, org);
  const role = await roleFor(user, org);
  let planned = false;
  if (taskId) {
    const approvals = await getDb().select().from(agentApprovals).where(and(eq(agentApprovals.organizationId,org),eq(agentApprovals.taskId,taskId),eq(agentApprovals.toolName,'request_execution_plan'),eq(agentApprovals.status,'approved'),gt(agentApprovals.expiresAt,new Date())));
...
export async function planEvidence(args: Record<string,unknown>, user: string, org: string) {
  const state = await readOnboarding(user,org);
