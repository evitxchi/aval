...
export async function pollInbox(org:string,provider:string,resourceId?:string) {
  if(!(POLL_PROVIDERS as readonly string[]).includes(provider))throw new Error('Use the signed webhook for this provider.');
  const {connection,credentials}=await connectedAccount(org,provider);
  ...
  for(const message of incoming.reverse()){
    ...
    if(inserted.length){
      ...
      if(message.at.getTime()>Date.now()-5*60000)await queueInboundTask(org,thread.id,message.id,message.body);
    }
  }
